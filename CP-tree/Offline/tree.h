...
private:
    ...
    offline_position& active_edge() const;
    void add_SL(ll node_id);
    bool walk_down(ll node_id);
    void st_extend(const offline_position& element);
    pair<ll, ll> save_leaves(ll node_id);
public:
    ll root;
    MiningNode *tree;
    offline_position *text;
    ll *to_terminator;
    unordered_map<ll, int> terminator2ListId;
    vector<ll> leaves;

    explicit MiningTree(vector<vector<offline_position>>& positions_list);
    ~MiningTree(){
        delete[] tree, text, to_terminator;
    }
};

MiningTree::MiningTree(vector<vector<offline_position>>& positions_list) : positions_list(positions_list){
    ...
    for(int i = 0;i < n_positions;i++){
        positions_list[i].push_back(offline_position(terminator));
    ...
    text = new offline_position[text_size];
    ...
    for(int i = 0;i < n_positions;i++){
        vector<offline_position>& positions = positions_list[i];
        for(offline_position& element : positions) st_extend(element);
    }
    save_leaves(root);
}
...
offline_position& MiningTree::active_edge() const {
...
void MiningTree::st_extend(const offline_position& element) {
    text[++pos] = element;
    needSL = 0;
    remainder++;
    while(remainder > 0){
