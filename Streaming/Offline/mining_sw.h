...
private:
    ...
    void add_result(OfflineResult &result, vector<int> &cluster, int begin_time, int end_time, ll m);
...
public:
    SW_Miner(ll length, vector<ll> &begin_ids, vector<int> &cars, MiningTree &tree): camera_length(length), begin_ids(begin_ids), cars(cars), mining_tree(tree){
        car_length = cars.size();
    }

    void mine(OfflineResult &result, ll m, ll k, double eps);
};

void SW_Miner::mine(OfflineResult &result, ll m, ll k, double eps) {
    ...
    for(int camera_idx = 0; camera_idx < camera_length; camera_idx++){
        vector<pair<int, int>> car2position; // record the offline_position of the car_id in the inverted index
        ...
        deque<int> window; // the sliding buffer
        vector<Group> groups;
...
void SW_Miner::get_clusters(int camera_idx, ll m, double eps, vector<pair<int, int>> &car_ids, vector<pair<int, int>> &cluster_marks) {
    offline_position* text = mining_tree.text;
...
void SW_Miner::add_result(OfflineResult &result, vector<int> &cluster, int begin_time, int end_time, ll m) {
