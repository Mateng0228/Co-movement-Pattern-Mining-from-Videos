        ...
        for(int group_idx = 0;group_idx < groups.size();group_idx++){
            ...
            for(int cluster_idx = begin_cluster_idx; cluster_idx <= end_cluster_idx; cluster_idx++){
                ...
                for(int item_idx = cluster_pair.first;item_idx <= cluster_pair.second;item_idx++){
                    ...
                    offline_position& pos = arr_positions[info.object][info.position_idx];
                    pos.group = group_idx;
...
void trajectory_simplification(vector<offline_position> *init_positions, int init_size, ll k, vector<vector<offline_position>>& positions_list, vector<int>& sub2init){
    for(int init_idx = 0;init_idx < init_size;init_idx++){
        vector<offline_position>& positions = init_positions[init_idx];
